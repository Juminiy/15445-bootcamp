...
public:
  ...
  Point(int x, int y) : x_(x), y_(y) {
    std::cout << "Custom constructor" << "{" << x << "," << y << "}" << "for the Point class is called.\n";
  }

  ~Point() {
    std::cout << "Default destructor" << "{" << x_ << "," << y_ << "}" << "for the Point class is called.\n";
  }
...
int main() {
  ...
  for (size_t i = 0; i < point_vector.size(); ++i) {
    std::cout << point_vector[i] << " \n"[i == point_vector.size()-1];
  }
