...
std::pair<std::vector<K>, std::vector<V>> MapVector(const std::unordered_map<K, V>& _m);
...
int main() {
  ...
  for(size_t i = 1; i < 16; i++) {
    uMap.insert({"myk"+std::to_string(i), i});
  }
  auto vecKV = MapVector(uMap);
  for(size_t i = 0; i < 16-1; i++) {
    std::cout << vecKV.first[i] << ", " << vecKV.second[i] << "\n";
  }
  return 0;
}
...
template <typename K, typename V>
std::pair<std::vector<K>, std::vector<V>> MapVector(const std::unordered_map<K, V>& _m) {
  std::vector<K> vecK; vecK.reserve(_m.size());
  std::vector<V> vecV; vecV.reserve(_m.size());
  for (const std::pair<K, V> &it : _m) {
    vecK.emplace_back(it.first);
    vecV.emplace_back(it.second);
  }
  return std::make_pair(std::move(vecK), std::move(vecV));
}