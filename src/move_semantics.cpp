...
void move_add_three_and_print(std::vector<int> vec) { // try to value, also be moved
...
void add_three_and_print(std::vector<int> vec) { // try to value 
  vec.push_back(3);
...
int main() {
  ...
  std::vector<int> stealing_ints = std::move(int_array); // int_array is null
  ...
  std::vector<int> const & lvalue_stealing_ints = std::move(stealing_ints);
  // lvalue_stealing_ints = std::vector<int>();
